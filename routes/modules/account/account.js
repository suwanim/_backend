...
const geninvoiceNumber = async () => {
  const _get_sql = "SELECT invoice_no FROM tb_marketing_invoice ORDER BY id DESC LIMIT 1"
  const _last_id = await funcs.getTable(_get_sql)
    if(_last_id===null||_last_id===undefined||_last_id===""||_last_id==="[]"){
        _last_id = 7000000;
    }
...
router.get("/geninvoicenumber", authen, async function (req, res, next) {
  try {
    const invoiceNumber = await geninvoiceNumber()
    console.log("invoiceNumber", invoiceNumber)
    const _newinvoiceNumber = parseInt(invoiceNumber[0].invoice_no) + 1

    res.send({ invoice_no: _newinvoiceNumber }).status(200)
  } catch (error) {
    console.log(error)
...
router.post("/invoice", authen, async function (req, res, next) {
  try {
    const _tableName = "tb_marketing_invoice" // tabel name in database
...
// router.put("/coline", authen, async function (req, res, next) {
//   try {
//     const _tableName = "tb_marketing_co_line" // tabel name in database
//     const _primaryKey = "id" //pk name
//     const _data = req.body // data from frontend
